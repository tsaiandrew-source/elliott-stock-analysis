const CACHE_VERSION = 'elliott-pwa-v25';
...
const CORE_ASSETS = [
  './offline.html', './index.html', './manifest.webmanifest',
  ...
  './chart-surface/universal-refresh-gex-consumer.js', './chart-surface/universal-refresh-gex-data.js'
...
self.addEventListener('activate', (event) => {
  event.waitUntil(caches.keys().then((keys) => Promise.all(keys.filter((key) => key.startsWith('elliott-pwa-') && ![CORE_CACHE, RUNTIME_CACHE].includes(key)).map((key) => caches.delete(key)))).then(() => self.clients.claim()));
});
